...
#        pydevd.settrace(host='192.168.1.103',port=5678)
    ...
    def on_clock(self):
        ...
        filename = "t_timestep_delay"
        file = open(filename, "a")
        file.write(str(on_clock_start- self.c37_end)+'\n')
        file.close()
        ...
        if (len(self.dataValues) != 0):
            ...
            der_OMEGASecondaryControlVariable = ( 2*(self.frequency-376.9911184)/6 * self.sec_en + self.sum_OMEGASecondaryControlVariable*4) * self.Ts*2 - 0.001*self.VoltageAngleDiff*self.VoltageRegulateDG*self.angle_en*self.sec_en* self.Ts/0.05
            ...
            der_ReactivePower = ( (-self.VoltageMagDiff)*self.VoltageRegulateDG /32.0  + 0.001*sum_ReactivePower) * self.Ts * self.sec_en
            
            #s = [(-self.VoltageMagDiff)*self.VoltageRegulateDG /32.0,   0.001*sum_ReactivePower]
            #print(str(s))
            ...
            time_modbus_delay  = on_clock_modbus_command_end - on_clock_modbus_command_start
            filename = "t_modbus_2way"
            file = open(filename, "a")
            file.write(str(time_modbus_delay)+'\n')
            file.close()
            
            time_calculation_delay  = on_clock_modbus_command_start - calculation_start
            filename = "t_calculation_delay"
            file = open(filename, "a")
            file.write(str(time_calculation_delay)+'\n')
            file.close()
        ...
        if otherId != self.ip: #self.uuid was used for general operation
            ...
            if self.ip == 112 :
                if otherId  == 111 or otherId  == 113:
                    self.dataValues[otherId] = (otherValue_OMEAG, otherValue_ReactivePower)
            if self.ip == 113 :
                if otherId  == 112 or otherId  == 114 or otherId  == 111:
                    self.dataValues[otherId] = (otherValue_OMEAG, otherValue_ReactivePower)
            if self.ip == 114 :
                if otherId  == 113:
                    self.dataValues[otherId] = (otherValue_OMEAG, otherValue_ReactivePower)     
    ...
    def on_relay_rx_phasorData(self):
        data = self.relay_rx_phasorData.recv_pyobj()  
        
        if (self.VoltageRegulateDG == 1):   
            self.VoltageMagDiff = data['mag_diff']
            self.VoltageAngleDiff = data['angle_diff'] 
            C37_start = data['time_arrive']
            C37_end = time.time()  
            self.c37_end = C37_end

            #try:
            time_c37_delay  = C37_end - C37_start
            filename = "t_cal_c37"
            file = open(filename, "a")
            file.write(str(time_c37_delay)+'\n')
            file.close()
