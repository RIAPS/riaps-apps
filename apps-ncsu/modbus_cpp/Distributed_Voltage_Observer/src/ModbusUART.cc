    ...
    namespace components {
        ...
        ModbusUART::ModbusUART(const py::object*  parent_actor     ,
            ...
            if (ret!=0) {
                ...
                component_logger()->info("ffComponent is using FIFO scheduler, priority: {}", x.sched_priority);
            }
        ...
        void ModbusUART::OnClock() {
            ...
            // Modbus is ready, sync the clocks
                ...
                if (regs_read == -1) {
                    ...
                    ts_diff.tv_nsec = after.tv_nsec - now.tv_nsec;

                    // Handling the carry
                    if (ts_diff.tv_nsec<0){
                        ts_diff.tv_sec--;
                        ts_diff.tv_nsec+=1000000000;//BILLION;
                    }

                    // Convert to nanosec for example. (or whatever representation we need)
