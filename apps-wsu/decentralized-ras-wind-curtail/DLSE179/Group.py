...
EXCEL_FILE = "utils/H_matrix_RTDS.xlsx"
...
I_ji = [1, None, None, 4, 5, None, 7, None, 9, 10, 11, None, 13, 14, None, 16, 17, None, None, None]

GROUP_MAP[4] = [V_mea, I_ij, I_ji]
...
class Group(Component):
    def __init__(self, group_no):
        ...
        self.V = None
        self.I = None
        self.group_name = "Group" + str(self.group_no)
    ...
    def get_config(self):
        with open("Group_config.txt", 'r') as f:
            for line in f:
                if len(line) == 0:
                    continue
                line_split = line.split(':')
                if line_split[0] == str(1):
                    cfg = line_split[1].strip()
                    if cfg.startswith("H"):
                        self.group_name = cfg.split("=")[1].strip()
                    if cfg.startswith("V_mea"):
                        self.V_mea = ast.literal_eval(cfg.split("=")[1].strip())
                    if cfg.startswith("I_ij"):
                        self.I_ij = ast.literal_eval(cfg.split("=")[1].strip())
                    if cfg.startswith("I_ji"):
                        self.I_ji = ast.literal_eval(cfg.split("=")[1].strip())
    ...
    def calculate(self):
        ...
        X = numpy.dot(prod2, Z)
        estimated_values = numpy.dot(H,X)
        self.logger.info(str(Z))
        self.logger.info(str(estimated_values))
        
        result = Z - estimated_values
        final_error = math.sqrt(sum([x ** 2 for x in result]))
        self.logger.info("FINAL RESULT " + str(final_error))

        V_estimate_dict = dict()

        for i in range(len(self.V_Mea)):
            v_index = self.V_mea[i]
            V_estimate_dict[v_index] = estimated_values[i] + estimated_values[i + len(self.V_mea)] * 1j
        
        #self.estimate_port.send_pyobj(estimate_dict)
