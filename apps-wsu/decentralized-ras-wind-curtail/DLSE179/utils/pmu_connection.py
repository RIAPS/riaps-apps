...
PMU_CONFIG_FILE = "utils/pmu_config.txt"
GROUP_CONFIG_FILE = "utils/group_config.txt"
...
class PMUConnection():
        ...
        def get_config(self):
            with open(PMU_CONFIG_FILE, 'r') as f:
                    PMUS = {}
                    PMU_PHASOR_MAP = {}
                    for line in f:
                        line_split = line.split(':')
                        if (line_split[0].isdigit()):
                            pmu_id = int(line_split[0])
                            cfg = line_split[1]
                            if cfg.startswith("Address"):
                                ip = cfg.split('=')[-1].strip()
                                PMUS[pmu_id] = [ip]
                            if cfg.startswith("Port"):
                                port = int(cfg.split('=')[-1].strip())
                                PMUS[pmu_id].append(port)
                            if cfg.startswith("V"):
                                v_list = ast.literal_eval(cfg.split('=')[-1].strip())
                                PMU_PHASOR_MAP[pmu_id] = [v_list]
                            if cfg.startswith("I"):
                                i_list = ast.literal_eval(cfg.split('=')[-1].strip())
                                PMU_PHASOR_MAP[pmu_id].append(i_list)
            ...
            with open(GROUP_CONFIG_FILE, 'r') as f:
                for line in f:
                        ...
                        if (line_split[0].isdigit()):
                            pmu_id = int(line_split[0])
                            cfg = line_split[1]
                            if cfg.startswith("PMU"):
                                ids = ast.literal_eval(cfg.split('=')[-1].strip())
                                PMU_IDS = ids
        ...
        def connect_pmu(self):
                ...
                if self.pdc is None:
                        ...
                        for pmu_id in PMU_IDS:
                                ...
                                pmu = Pdc(pmu_ip = PMU[0], pmu_port = PMU[1], pdc_id = pmu_id)
                                pmu.run()
                                self.pdc[pmu_id] = pmu
