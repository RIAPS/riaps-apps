...
namespace tsyncgpio {
    namespace components {
        ...
        TscaGpio::TscaGpio(_component_conf &config, riaps::Actor &actor)
            ...
            if (!m_pps_output) {
                _logger->error("unable to request gpio pin, trying again");
            }

            // Repeat to get the pin enabled
            m_pps_output  = libsoc_gpio_request(PPS_OUTPUT, LS_GPIO_SHARED);
            sleep(1);

            // Second call finds the gpio pin already exported, which is not a problem
            if (!m_pps_output) {
                _logger->error("unable to request gpio pin, may already be exported");
            }
        ...
        void TscaGpio::OnClock(riaps::ports::PortBase *port) {
             ...
             **/
                ...
                if (GetLeaderId(gid) != GetCompUuid()) {
                     ...
                     */
                    std::string proposeId = ProposeAction(gid, TOGGLE_ACTION_ID, now);
                }
            }
        }
...
//            }
        ...
        void TscaGpio::OnAnnounce(const riaps::groups::GroupId &groupId,
            ...
            if (accepted && m_accepted.find(proposeId) != m_accepted.end() &&
                m_pendingActions.find(TOGGLE_ACTION_ID) == m_pendingActions.end()) {
                ...
                m_pendingActions.insert(TOGGLE_ACTION_ID);
        ...
        void TscaGpio::ActionA(const uint64_t timerId) {
             ...
             */
            m_pendingActions.erase(TOGGLE_ACTION_ID);
        }
